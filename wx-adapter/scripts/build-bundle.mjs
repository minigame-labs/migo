...
//
// The adapter is idempotent (guarded by `globalThis.__migoWxAdapterInjected`),
...
await build({
  ...
  // isolated; only the `globalThis.wx` write leaks out, which is exactly
  // what we want.
  platform: "neutral",
  target: ["es2020"],
  outfile: resolve(root, "dist/migo-wx-adapter.bundle.js"),
  sourcemap: false,
  legalComments: "none",
  // Banner makes the output self-describing if a developer looks at it.
  banner: {
    js: "/* @minigame-labs/migo-wx-adapter — IIFE bundle. Source: wx-adapter/src/index.js */",
  },
});

console.log("built dist/migo-wx-adapter.bundle.js");
