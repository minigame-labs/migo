...
class Event {
  constructor(type, eventInitDict = { __proto__: null }) {
    ...
    utility.requiredArguments(
      arguments.length,
      1,
      "Failed to construct 'Event'",
    );
    type = utility.converters.DOMString(
      type,
      "Failed to construct 'Event'",
      "Argument 1",
    );
  ...
  set cancelBubble(value) {
    this[_stopPropagationFlag] = utility.converters.boolean(value);
  }
  ...
  set returnValue(value) {
    if (!utility.converters.boolean(value)) {
...
function addEventListenerOptionsConverter(V, prefix) {
  ...
  if (signal !== undefined) {
    options.signal = utility.converters.AbortSignal(
      signal,
      prefix,
...
class EventTarget {
  ...
  addEventListener(
    ...
    utility.requiredArguments(arguments.length, 2, prefix);
...
utility.configureInterface(EventTarget);
