...
3. **CI freshness gate.** `scripts/check-snapshot-freshness.sh` recomputes the
   ...
   bumping deno_core without refreshing the snapshots is caught.

4. **Regeneration workflow** (when the gate goes red):
   - **x86_64:** trigger `build-snapshot.yml` manually (Actions → *Build V8
     Snapshot* → **Run workflow**). It regenerates on an emulator, writes the
     manifest, and opens a `bot/regen-x86_64-snapshot` PR (skips if the bytes are
     unchanged). Review + merge. The slow generate runs *only* on this manual
     dispatch, never on push.
   - **arm64:** regenerate on a real device: `scripts/gen-snapshot.sh arm64
     --device <serial>`, then commit `SNAPSHOT-aarch64.bin` (LFS) + its manifest
     (can go in the same PR).
...
- **`freshness`** (host, `ubuntu-latest`): runs `check-snapshot-freshness.sh`
  against the committed snapshots on every snapshot-relevant push — the fast
  (~6 s) alarm. This is the only job that runs on push.
