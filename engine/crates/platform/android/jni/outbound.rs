...
pub fn unzip(host_id: i32, request_id: i32, zip_path: &str, dest: &str) -> Result<(), String> {
    with_env(|env| {
        let cache = JAVA_METHOD_CACHE
            .get()
            .ok_or("NativeExports class cache not initialized")?;
        let method_id = cache.get_method_id("unzip").ok_or("Method ID not found")?;

        let class = cache.class();

        let j_zip_path = env
            .new_string(zip_path)
            .map_err(|e| format!("Failed to create JString for zip_path: {e}"))?;
        let j_dest = env
            .new_string(dest)
            .map_err(|e| format!("Failed to create JString for dest: {e}"))?;

        let args = &[
            jvalue { i: host_id },
            jvalue { i: request_id },
            jvalue {
                l: j_zip_path.into_raw(),
            },
            jvalue {
                l: j_dest.into_raw(),
            },
        ...
        unsafe {
            env.delete_local_ref(jni::objects::JObject::from_raw(args[2].l))
                .ok();
            env.delete_local_ref(jni::objects::JObject::from_raw(args[3].l))
                .ok();
        }
        ...
        match result {
            ...
            Err(e) => {
                ...
                Err(format!("Failed to call unzip: {}", e))
            }
        }
    })
}
