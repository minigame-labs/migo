//! Build script for the runtime-v8 crate.
//!
//! Selects and validates kind/profile/platform-bound V8 startup snapshots.
...
//! in an android-aarch64 V8, a linux-x86_64 snapshot only in a linux-x86_64 V8,
//! and neither loads in the other even when `CARGO_CFG_TARGET_ARCH` agrees —
//! each OS links a distinct V8 archive (see `v8_target_dir` below), so "same
//! arch" is not "same V8". Therefore:
//!
//!   * We keep one snapshot per product, OS and arch under
//!     `snapshots/SNAPSHOT-<profile>-<os>-<arch>.bin` and pick the one matching
//!     `(CARGO_CFG_TARGET_OS, CARGO_CFG_TARGET_ENV, CARGO_CFG_TARGET_ARCH)`.
//!     This makes a single multi-ABI `build-aar.sh` invocation embed the
//!     *correct* snapshot into each `.so` (embedding the wrong one would crash
//!     at startup with a V8 magic-number mismatch, which is also why the
//!     `v8_archive_sha256` recorded in the manifest is checked against the
//!     actual archive on this OS, not just the arch word).
//!   * We only embed for the OS/env combinations `v8_target_dir` recognizes
//!     (android, linux-gnu, OpenHarmony, windows-msvc). Any other host
//!     (`cargo test` on macOS, an unlisted env) always loads extension JS from
//!     source — embedding a foreign-platform snapshot would crash on load.
//!
//! Snapshots are generated by `migo-snapshot-gen` cross-compiled to the target
//! platform and run there (device/emulator for Android and OpenHarmony,
//! natively for Linux and Windows); see `tools/snapshot-gen`.
...
fn main() {
    ...
    // The OS word used in snapshot filenames, and the directory under
    // engine/third_party/rusty_v8/ that names this platform's V8 archive.
    // `target_os` alone cannot distinguish Linux from OpenHarmony — Rust
    // reports both as `target_os = "linux"` with `target_env` carrying "ohos"
    // — so both are consulted. Anything else (macOS, an unlisted env) has no
    // committed V8 archive to embed against and always loads JS from source.
    let (os_word, v8_target_dir) = match (target_os.as_str(), target_env.as_str()) {
        ("android", _) => ("android", format!("{target_arch}-linux-android")),
        ("linux", "ohos") => ("ohos", format!("{target_arch}-linux-ohos")),
        ("linux", _) => ("linux", format!("{target_arch}-linux-gnu")),
        ("windows", _) => ("windows", format!("{target_arch}-pc-windows-msvc")),
        _ => return,
    };
    ...
    let repo_root = manifest_dir
        .ancestors()
        .nth(3)
        .expect("runtime-v8 must live under <repo>/engine/crates");
    ...
    let snapshot_path = manifest_dir.join("snapshots").join(format!(
        "SNAPSHOT-{product_profile}-{os_word}-{target_arch}.bin"
    ));
    ...
    let worker_snapshot_path = manifest_dir.join("snapshots").join(format!(
        "SNAPSHOT-worker-{product_profile}-{os_word}-{target_arch}.bin"
    ));
