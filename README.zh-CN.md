...
`migo.*` 是引擎唯一安装的原生能力面。现有内容期望的其他一切,都是叠加在其上的独立、可组合适配层——按内容实际需要,选零个、一个或多个：

- **[`adapter/`](adapter/)** —— 浏览器风格的 BOM/DOM 层(`window`、`document`、`Image`、`XMLHttpRequest` 等),给假设自己跑在浏览器环境里的引擎用(Cocos、Egret、Laya、Pixi、原生 Canvas/WebGL)。
- **[`wx-adapter/`](wx-adapter/)** —— 发布 `globalThis.wx`,映射到 `migo.*`,给按微信小游戏全局对象写的内容用(未经改动的微信小游戏源码,或形态类似的小游戏平台)。

这两层各自只碰互不重叠的全局对象,可以自由组合——面向微信小游戏平台构建的内容通常两者都要(用 `wx.*` 调平台接口,用 DOM shim 支撑引擎内部逻辑),这也正是为什么真实的微信小游戏引擎自己也会附带这样一层适配。以后再来一个平台(比如快游戏联盟成员),还是同一个配方:加一层适配,引擎不用改。
