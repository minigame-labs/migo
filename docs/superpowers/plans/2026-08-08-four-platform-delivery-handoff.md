...
1. **The remaining phase-0 epics, sorted by what they actually need.** "Most have
   ...
   |---|---|---|
   | 0.9/A9 | **execute, tasks 1–6 of 12 done** | plan `2026-08-02-runtime-restart-generation-boundary.md`; its §6.6 authority is *not* superseded, checked. **Six** of the plan's own premises were wrong — see 0.9. Tasks 5 and 6 are now whole: every id space is the Host's, and every Android result echoes the id it was given. **Next is Task 7**, and it is the one that makes any of this change behaviour — until something *compares* the generation before invoking JavaScript, tasks 1–6 are plumbing. Task 3's step-3 fallback deletion is still not unblocked, and 0.9 says exactly what would unblock it (an audit that no host can produce an id-less deferred result, plus payment's two Midas branches) |
