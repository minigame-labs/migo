...
1. **The remaining phase-0 epics, sorted by what they actually need.** "Most have
   ...
   |---|---|---|
   | 0.9/A9 | **execute, tasks 1–4 and 6's JSON half done; 5 half done** | plan `2026-08-02-runtime-restart-generation-boundary.md`; its §6.6 authority is *not* superseded, checked. Task 3 landed as a **subset**: the fallback its step 3 deletes is what Task 6 makes safe to delete, and deleting early means promises that never settle. **Four** of the plan's own premises were wrong — see 0.9. **Next is task 5's UI half**, and 0.9 carries the survey: modal / action sheet / app-authorize genuinely send no id (`shift()`), while `openSystemBluetoothSetting` is *given* one and drops it. Only Android implements those four service methods, so the blast radius is one file per layer, and `profile_contract.rs:151-152` fails loudly if one side widens alone. Then task 6's `InteractionUI` half, which depends on it |
