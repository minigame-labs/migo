...
1. **The remaining phase-0 epics, sorted by what they actually need.** "Most have
   ...
   |---|---|---|
   | 0.9/A9 | **execute, tasks 1–4 of 12 done** | plan `2026-08-02-runtime-restart-generation-boundary.md`; its §6.6 authority is *not* superseded, checked. Task 3 landed as a **subset**: the fallback its step 3 deletes is what Task 6 makes safe to delete, and deleting early means promises that never settle. Two of the plan's own code blocks were wrong — see 0.9. Resume at **Task 5**, the big one: 11 files reaching into Rust services and Android. Tasks 3 and 4 both landed as subsets — every FIFO fallback the plan deletes is one task 6 has not yet made safe to delete, so 6 may want to come before 5 |
