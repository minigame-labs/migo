...
1. **The remaining phase-0 epics, sorted by what they actually need.** "Most have
   ...
   |---|---|---|
   | 0.9/A9 | **execute, tasks 1–4 of 12 done** | plan `2026-08-02-runtime-restart-generation-boundary.md`; its §6.6 authority is *not* superseded, checked. Task 3 landed as a **subset**: the fallback its step 3 deletes is what Task 6 makes safe to delete, and deleting early means promises that never settle. Two of the plan's own code blocks were wrong — see 0.9. Task 6 started: location and scan code echo their ids. **Order finding:** 6 before 5 for families that are *given* an id and drop it, but modal/action sheet have no id at all (`_pendingModals.shift()`), so their half of 6 depends on 5. Next: `ImageApiManager`, `VideoManager`, `ResultProxyActivity` |
