...
rm -rf "$PREFIX"
mkdir -p "$PREFIX/include" "$PREFIX/lib" "$PREFIX/bin" "$PREFIX/cmake/migo" "$PREFIX/share/migo"
...
info "NOTE: CMake package, NuGet, and the V8 component-manifest provenance are the"
info "      next steps; this stages a linkable, runnable migo.dll + headers first."
