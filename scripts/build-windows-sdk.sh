...
# 0.1.0 was published before the C ABI had a Windows platform layer: the DLL
# loaded, exported everything, and reported it could attach no surface kind. A
# released version names a fixed set of bytes, so the fix ships as 0.1.1 rather
# than as different bytes under the version some consumer may already hold.
MIGO_SDK_VERSION="0.1.1"
