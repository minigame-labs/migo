...
#
# Honours MIGO_HOST_V8_DIR when set, accepting either layout under it, then the
# in-repo fetch, then a sibling rusty_v8_src build.
host_v8_resolve() {
    local repo_root="$1"
    local candidates=()

    if [[ -n "${MIGO_HOST_V8_DIR:-}" ]]; then
        # Explicit wins, and both layouts are accepted under it: pointing this at
        # a directory holding the pair and having it demand `obj/` underneath is
        ...
        # Preferred: the fetch verifies the archive against the manifest committed
        # beside it, so this path carries provenance a local build does not.
    ...
    for candidate in "${candidates[@]}"; do
        ...
        if [[ -f "$archive_dir/librusty_v8.a" && -f "$binding_dir/src_binding.rs" ]]; then
            HOST_V8_ARCHIVE="$archive_dir/librusty_v8.a"
            HOST_V8_BINDING="$binding_dir/src_binding.rs"
            # An LFS pointer is a file of the right name and the wrong size, and it
            # fails later as an inscrutable link error. `stat -c %s` and not a
            # symlink-following check on purpose: a symlink reports its own size
            # and would read as a pointer.
